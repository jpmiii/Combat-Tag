...
public class PlayerDataManager {
	 ...
	 *            with which to get the data from
	 */
	public static void savePlayerData(String directory,
			PlayerDataContainer playerContainer) {
		File file = new File(directory + File.separator
				+ playerContainer.getPlayerName());
		SettingsHelper settings = new SettingsHelper(file, "CombatTag");
		settings.setProperty("Health",
				String.valueOf(playerContainer.getHealth()));
		settings.setProperty("Experience",
				String.valueOf(playerContainer.getExp()));
		settings.setProperty("Should-Be-Punished",
				String.valueOf(playerContainer.shouldBePunished()));
		String[] items;
	 ...
	 * 
	 * @param directory
	 *            with which to load the file
	 * @param playerName
	 *            name of the player and consiquently the file
	 * @return PlayerDataContainer which contains the data of the player
	 */
	public static PlayerDataContainer loadPlayerData(String directory,
			String playerName) {
		...
		boolean punish = Boolean.valueOf(settings
				.getProperty("Should-Be-Punished"));
	...
	/**
	 * Converts the itemstack array to a string array seperated by : the flormat
	 * is id:durability:amount
	 * 
	 * @param items
	 * @return
	 */
