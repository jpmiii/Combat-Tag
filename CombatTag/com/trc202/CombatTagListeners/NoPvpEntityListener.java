...
public class NoPvpEntityListener implements Listener {
	...
	public void onEntityDamage(EntityDamageEvent EntityDamaged) {
		...
		if (EntityDamaged instanceof EntityDamageByEntityEvent) {
			...
			if ((dmgr instanceof Player) && (e.getEntity() instanceof Player)) {// Check
																				// to
																				// see
																				// if
																				// the
				...
				if (damager != tagged && damager != null) {
					for (String disallowedWorlds : plugin.settings
							.getDisallowedWorlds()) {
						if (damager.getWorld().getName()
								.equalsIgnoreCase(disallowedWorlds)) {
							// Skip this tag the world they are in is not to be
							// tracked by combat tag
							return;
						}
					}
	...
	public void onEntityDeath(EntityDeathEvent event) {
		...
		// if Player died with a tag duration, cancel the timeout and remove the
		// data container
	...
	public void onPlayerDeath(Player deadPlayer) {
		if (plugin.hasDataContainer(deadPlayer.getName())) {
			PlayerDataContainer deadPlayerData = plugin
					.getPlayerData(deadPlayer.getName());
